...
from finalaverg_randomsampling import theano_word_model
random.seed(1)
np.random.seed(1)
...
def LModel(eta,batchsize,dSize,relSize, updatewords):
	...
	params.frac = 1.0
	params.outfile = 'Model_FA_randomSamp'+'_eta_'+str(eta)+'_dSize_'+ str(dSize) + '_batchsize_'+ str(batchsize) + '_relSize_'+ str(relSize) + '_trainSize_'+str(trainSize[0]) + '_updatewords_' + str(updatewords)
