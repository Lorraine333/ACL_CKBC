...
#                                  params.updateword, params.eta, params.peephole, params.outgate)
	...
	#l_lstm = lasagne.layers.FeaturePoolLayer(l_lstm3, pool_size= seqlen, axis =1, pool_function=theano.tensor.max)
        #l_lstm = lasagne.layers.SliceLayer(l_lstm3, -1, 1)
	#l_out = lasagne.layers.DenseLayer(l_lstm, relsize,nonlinearity = lasagne.nonlinearities.tanh)
	#l_out = lasagne.layers.DenseLayer(l_lstm, relsize)
	l_out0 = lasagne.layers.SliceLayer(l_out, -1, 1)        

        embg10 = lasagne.layers.get_output(l_out, {l_in:g1batchindices, l_mask:g1mask})
        embg10 = (embg10 * g1mask[:,:,None]).sum(axis=1)
        embg10 = embg10 / g1mask.sum(axis=1)[:, None]
        
        embg20 = lasagne.layers.get_output(l_out, {l_in:g2batchindices, l_mask:g2mask})
        embg20 = (embg20 * g2mask[:,:,None]).sum(axis=1)
        embg20 = embg20 / g2mask.sum(axis=1)[:, None]
        
        embp10 = lasagne.layers.get_output(l_out, {l_in:p1batchindices, l_mask:p1mask})
        embp10 = (embp10 * p1mask[:,:,None]).sum(axis=1)
        embp10 = embp10 / p1mask.sum(axis=1)[:, None]
       
        embp20 = lasagne.layers.get_output(l_out, {l_in:p2batchindices, l_mask:p2mask})
        embp20 = (embp20 * p2mask[:,:,None]).sum(axis=1)
        embp20 = embp20 / p2mask.sum(axis=1)[:, None]
	

	embg1_ = lasagne.layers.get_output(l_out0, {l_in:g1batchindices, l_mask:g1mask})
        embg2_ = lasagne.layers.get_output(l_out0, {l_in:g2batchindices, l_mask:g2mask})
        embp1_ = lasagne.layers.get_output(l_out0, {l_in:p1batchindices, l_mask:p1mask})
        embp2_ = lasagne.layers.get_output(l_out0, {l_in:p2batchindices, l_mask:p2mask})
	
	embg1 = T.concatenate([embg10, embg1_],axis =1)
        embg2 = T.concatenate([embg20, embg2_],axis =1)
        embp1 = T.concatenate([embp10, embp1_],axis =1)
        embp2 = T.concatenate([embp20, embp2_],axis =1)


	self.a2 = np.random.uniform(low = -0.01, high = 0.01, size = [4*memsize,relsize])
        self.a3 = np.random.uniform(low = -0.0, high = 0.0, size = [relsize,])
	self.w = theano.shared(self.a2)
        self.b = theano.shared(self.a3)
	

	#l_trans = lasagne.layers.InputLayer(shape=(None, 2*memsize))
	#l_out1 = lasagne.layers.DenseLayer(l_trans, relsize)	

        #embg1 = lasagne.layers.get_output(l_out1, {l_trans:embg10})
	embg1 = T.tanh(T.dot(embg1,self.w)+self.b.dimshuffle('x', 0))
	embg2 = T.tanh(T.dot(embg2,self.w)+self.b.dimshuffle('x', 0))
	embp1 = T.tanh(T.dot(embp1,self.w)+self.b.dimshuffle('x', 0))
	embp2 = T.tanh(T.dot(embp2,self.w)+self.b.dimshuffle('x', 0))
	#embg2 = lasagne.layers.get_output(l_out1, {l_trans:embg20})	
	#embp1 = lasagne.layers.get_output(l_out1, {l_trans:embp10})
        #embp2 = lasagne.layers.get_output(l_out1, {l_trans:embp20})
	...
	self.all_params.append(self.Rel)
        ...
        if updatewords:
            ...
            self.train_function = theano.function([g1batchindices, g2batchindices, p1batchindices, p2batchindices,
                             g1mask, g2mask, p1mask, p2mask, r, pr], [cost, cost1], updates=updates, on_unused_input='warn')
        else:
	...
	result1 =0
        ...
        try:
		    ...
		    #print 'Epoch ', (eidx+1), 'Cost ', cost , 'Pure Cost ', cost1
	            ...
	            COPA, accurancy1, accruancy2, threshold = self.evaluate()
		    if COPA > result0:
			result0 = COPA
			recordtime = eidx
			...
			conceptime = eidx
		    #print COPA
		    text_file.write( "\nCOPA Result: %f dev1 %f  dev2 %f  threshold %f\n\n" % (COPA, accurancy1, accruancy2, threshold))
		    text_file.write( ' ')
